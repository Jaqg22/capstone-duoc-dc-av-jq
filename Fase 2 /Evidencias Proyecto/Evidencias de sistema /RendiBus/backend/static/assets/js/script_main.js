...
function openNativeMobileCamera() {
    ...
    input.onchange = function(event) {
        ...
        if (files && files.length > 0) {
            ...
            showMessage('📤 Subiendo imagen a Azure...', 'info');
            
            // Subir directamente a Azure
            const formData = new FormData();
            formData.append('imagen', file);
            
            fetch('/api/upload_imagen/', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            .then(result => {
                if (result.success && result.planilla_id) {
                    console.log('✅ Imagen subida, ID:', result.planilla_id);
                    
                    // Guardar ID para manual.html
                    localStorage.setItem('planillaIdPendiente', result.planilla_id);
                    localStorage.setItem('imagenNombre', file.name);
                    localStorage.setItem('tiempoOCR', Date.now().toString());
                    
                    showMessage('✅ Redirigiendo...', 'success');
                    console.log('🚀 Redirigiendo a /manual/...');
...
async function capturePhoto() {
    ...
    try {
        ...
        // Convertir canvas a blob y subir directamente
        canvas.toBlob(async function(blob) {
            ...
            try {
                ...
                console.log('📄 Archivo creado:', `${(file.size / 1024).toFixed(2)} KB`);
...
function selectFromGallery() {
    ...
    input.onchange = function(event) {
        ...
        if (file) {
            ...
            // Convertir a DataURL sin comprimir
            ...
            reader.onload = function(e) {
                try {
                    ...
                    alert('La imagen es muy grande. Intenta con una imagen más pequeña.');
                }
            };
