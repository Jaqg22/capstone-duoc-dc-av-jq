...
async function capturePhoto() {
    ...
    try {
        ...
        console.log('🚨 VERSIÓN ACTUALIZADA - COMPRESIÓN DE IMAGEN');
        ...
        if (width > maxWidth || height > maxHeight) {
            ...
            console.log(`📐 Redimensionando: ${canvas.width}x${canvas.height} → ${width}x${height}`);
            ...
            console.log('📄 Imagen comprimida:', {
                filename: filename,
                originalSize: `${(canvas.width * canvas.height / 1024).toFixed(2)} KB aprox`,
            ...
            try {
                ...
                console.log('🚀 EJECUTANDO REDIRECCIÓN A /preview/...');
                
                // Navegar a preview
                window.location.href = '/preview/';
            } catch (storageError) {
                console.error('❌ Error guardando en localStorage:', storageError);
                alert('La imagen es muy grande. Por favor intenta con una imagen más pequeña.');
            }
        } else {
            // Imagen ya es pequeña, usar directamente
            const dataURL = canvas.toDataURL('image/jpeg', 0.85);
            const filename = `captura_${Date.now()}.jpg`;
            
            console.log('📄 Imagen capturada sin redimensionar:', {
                filename: filename,
