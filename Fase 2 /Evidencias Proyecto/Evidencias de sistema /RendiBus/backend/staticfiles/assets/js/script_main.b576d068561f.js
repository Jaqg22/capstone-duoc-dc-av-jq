...
function openNativeMobileCamera() {
    ...
    input.onchange = function(event) {
        ...
        if (files && files.length > 0) {
            ...
            // Comprimir y guardar en localStorage
            console.log('💾 Comprimiendo imagen...');
            showMessage('📸 Comprimiendo imagen...', 'info');
            
            compressImage(file, 2, 1920).then(compressedFile => {
                ...
                reader.onload = function(e) {
                    try {
                        ...
                        alert('Error al procesar imagen. La imagen puede ser muy grande.');
                    }
                };
...
async function capturePhoto() {
    ...
    try {
        ...
        console.log('🚨 Comprimiendo imagen capturada...');
        showMessage('📸 Comprimiendo imagen...', 'info');
        ...
        canvas.toBlob(async function(blob) {
            ...
            try {
                ...
                const originalFile = new File([blob], `captura_${Date.now()}.jpg`, { type: 'image/jpeg' });
                console.log('📄 Archivo original:', `${(originalFile.size / 1024).toFixed(2)} KB`);
                
                // Comprimir usando la función existente
                const compressedFile = await compressImage(originalFile, 2, 1920);
                ...
                reader.onload = function(e) {
                    try {
                        ...
                        alert('Error guardando imagen. Intenta de nuevo con una imagen más pequeña.');
                    }
                };
                reader.onerror = function() {
                    console.error('❌ Error leyendo archivo comprimido');
                    alert('Error al leer archivo');
                };
                reader.readAsDataURL(compressedFile);
                
            } catch (compressionError) {
                console.error('❌ Error comprimiendo:', compressionError);
                alert('Error al comprimir imagen');
            }
        }, 'image/jpeg', 0.9);
