...
// ==========================================
// FUNCIONES DE INDEXEDDB PARA IMÁGENES GRANDES
// ==========================================

function openImageDB() {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open('RindeBusImages', 1);
...
async function saveImageToIndexedDB(blob, filename) {
    return new Promise(async (resolve, reject) => {
        try {
            console.log('📦 [IndexedDB] Iniciando guardado:', filename, `${(blob.size / 1024).toFixed(2)} KB`);
            ...
            request.onsuccess = () => {
                console.log('✅ [IndexedDB] Imagen guardada exitosamente:', filename);
                resolve(true);
            };
            
            request.onerror = (error) => {
                console.error('❌ [IndexedDB] Error en request.put:', error);
                reject(error);
            };
            
            transaction.oncomplete = () => {
                console.log('✅ [IndexedDB] Transacción completada');
            };
            
            transaction.onerror = (error) => {
                console.error('❌ [IndexedDB] Error en transacción:', error);
                reject(error);
            };
...
function openNativeMobileCamera() {
    ...
    input.onchange = function(event) {
        ...
        if (files && files.length > 0) {
            ...
            compressImage(file, 2, 1920).then(async compressedFile => {
                console.log('✅ Comprimido:', `${(compressedFile.size / 1024).toFixed(2)} KB`);
                
                try {
                    // Guardar en IndexedDB
                    const saved = await saveImageToIndexedDB(compressedFile, compressedFile.name);
                    
                    if (saved) {
                        localStorage.setItem('tempImageName', compressedFile.name);
                        localStorage.setItem('tempImageSize', compressedFile.size);
                        localStorage.setItem('tempImageTimestamp', Date.now().toString());
                        
                        console.log('✅ Guardado, navegando a /preview/');
                        window.location.href = '/preview/';
                    } else {
                        alert('Error guardando imagen. Intenta de nuevo.');
                    }
                } catch (error) {
...
async function capturePhoto() {
    ...
    try {
        ...
        // Cerrar modal de cámara primero
        closeCameraModal();
        
        console.log('🚨 Procesando imagen capturada...');
        showMessage('📸 Procesando imagen...', 'info');
        
        // Convertir canvas a blob y guardar en IndexedDB
        canvas.toBlob(async function(blob) {
            if (!blob) {
                console.error('❌ Error creando blob');
                alert('Error procesando la captura');
                return;
            }
            
            console.log('📄 Blob creado:', `${(blob.size / 1024).toFixed(2)} KB`);
            ...
            try {
                ...
                if (saved) {
                    ...
                    localStorage.setItem('tempImageName', compressedFile.name);
                    localStorage.setItem('tempImageSize', compressedFile.size.toString());
                    localStorage.setItem('tempImageTimestamp', Date.now().toString());
                    
                    console.log('✅ Metadatos guardados en localStorage');
                    console.log('🚀 Navegando a /preview/...');
                    
                    showMessage('✅ Redirigiendo a vista previa...', 'success');
                    ...
                    console.error('❌ No se pudo guardar en IndexedDB');
                    alert('Error guardando imagen. Intenta de nuevo.');
                }
