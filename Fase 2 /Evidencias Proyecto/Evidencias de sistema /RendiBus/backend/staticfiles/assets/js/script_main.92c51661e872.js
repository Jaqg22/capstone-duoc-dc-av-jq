...
async function capturePhoto() {
    ...
    try {
        ...
        // Convertir canvas directamente a DataURL (SIN comprimir)
        const dataURL = canvas.toDataURL('image/jpeg', 0.92);
        const filename = `captura_${Date.now()}.jpg`;
        
        console.log('📄 DataURL generado:', `${(dataURL.length / 1024).toFixed(2)} KB`);
        ...
        try {
            ...
            console.error('❌ Error en localStorage (imagen muy grande):', storageError);
            alert('La imagen es muy grande para procesarla. Intenta tomar la foto más lejos o con menos zoom.');
        }
