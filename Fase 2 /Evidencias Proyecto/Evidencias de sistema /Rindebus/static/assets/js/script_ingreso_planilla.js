...
function uploadPlanilla() {
    ...
    showMessage('Enviando planilla...', 'info');
    
    // Crear un archivo JSON con los datos del formulario para simular imagen
    const formData = new FormData(form);
    const planillaData = Object.fromEntries(formData);
    const jsonBlob = new Blob([JSON.stringify(planillaData, null, 2)], { type: 'application/json' });
    const imageFile = new File([jsonBlob], `planilla_manual_${Date.now()}.json`, { type: 'application/json' });
    
    // Crear FormData para envío
    const finalFormData = new FormData();
    finalFormData.append('imagen', imageFile);
    ...
    .then(async response => {
        ...
        if (response.ok) {
            console.log('✅ Planilla enviada exitosamente:', result);
    ...
    .catch(error => {
        console.error('❌ Error enviando planilla:', error);
        showMessage('❌ Error al cargar la planilla: ' + error.message, 'error');
    });
}
