...
function initializeCropFrame(detectedCoords = null) {
    ...
    // Si hay coordenadas detectadas por OpenCV, usarlas
    if (detectedCoords && detectedCoords.x !== undefined) {
        setCropFrameFromOpenCVCoords(detectedCoords, imageRect, containerRect);
    } else {
...
// Función para establecer el marco desde coordenadas de OpenCV
function setCropFrameFromOpenCVCoords(coords, imageRect, containerRect) {
    const frame = document.getElementById('crop-frame');
    const image = document.getElementById('crop-image');
    ...
    // Convertir coordenadas de OpenCV a píxeles en la imagen mostrada
    ...
    console.log('🎯 Marco ajustado con coordenadas OpenCV:', {
        original: coords,
...
function applyAutoDetection() {
    if (currentCoordinates) {
        ...
        if (image && container) {
            const imageRect = image.getBoundingClientRect();
            const containerRect = container.getBoundingClientRect();
            setCropFrameFromOpenCVCoords(currentCoordinates, imageRect, containerRect);
        }
    } else {
